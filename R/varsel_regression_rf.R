# https://raw.githubusercontent.com/USEPA/LakeTrophicModelling/master/R/varsel_regression_rf.R

#' Variable Selection with Regression RF
#' 
#' Similar approach to varSelRF, but for regresssion.  Use full model to rank
#' variables based on %IncMSE or gini. Step through that list with most important first
#' run RF, store variables and %MSE of model.
#' @param y response a vector 
#' @param x predictors a data.frame
#' @param ... options to pass to randomForest; default (importance = FALSE) ranks on gini; to rank on %IncMSE
#' use "importance = TRUE".
#' @export
#' @import randomForest
#' @examples
