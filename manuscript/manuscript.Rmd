...
An advantage of this model is that we are now able to forecast and backcast lake photic zone temperature.  While there are numerous lakes that have substantial historical temperature data, these data are not consistently available for large regions.  This inhibits our ability to make large scale predictions about changes to lake dynamics (e.g., stratification and mixing) and ecological risk (e.g., anticipated changes in cyanobacteria bloom frequency).  At the continental scale, essentially, we could identify area that might have a higher ecological risk.  Yet if there was an individual lake of concern that lack temperature, this model would be as effective.  
